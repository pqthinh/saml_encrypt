...
function decodeSamlRedirect(encoded, cb) {
  ...
  var deflated = new Buffer.from(decodeURIComponent(encoded), "base64");
...
function encodeSamlPost(input, cb) {
  ...
  return cb(null, new Buffer.from(input, "base64"))
}
...
function decodeSamlPost(encoded, cb) {
  ...
  return cb(null, new Buffer.from(input, "base64").toString('ascii'));
}
